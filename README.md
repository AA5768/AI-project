...
[![CI](https://github.com/AA5768/AI-project/actions/workflows/ci.yml/badge.svg)](https://github.com/AA5768/AI-project/actions/workflows/ci.yml)

Ask a question across 22 meeting transcripts and 15 Office documents. Every
sentence of the answer carries the source it came from, down to the line, slide
or cell range. When the corpus cannot actually answer, the system says so and
names the person the record says to ask — or admits that it cannot name anyone.
...
[![Ask screen: an answered question with per-claim citations](docs/images/ask-answered.png)](docs/images/ask-answered.png)

<sub>Confidence **0.82**. Four claims, each carrying the citations for *that*
sentence, over three sources of visibly different quality.</sub>

## What it does when it should not answer

The interesting part of a system like this is not the answer. It is the two
other things it can do. Both of these are real screenshots of the running app
against the corpus in this repo:

| Routed to a person | Routed to nobody |
|---|---|
| [<img src="docs/images/routed-to-person.png" width="410">](docs/images/routed-to-person.png) | [<img src="docs/images/routed-to-nobody.png" width="410">](docs/images/routed-to-nobody.png) |
| Confidence **0.28**. The corpus talks about particle adders at length but no document states a nine-pass figure, so the question goes to **Ingrid Lund** — found by walking `chunks → documents → document_people → people`, never by asking a model who would know. The rationale quotes the line that chose her. | Confidence **0.32**. Nothing in the corpus is about parental leave, so **no one is named at all**. The gap is still recorded and still lands on the review queue. |

That second column is the one worth dwelling on. Naming whoever happens to sit
nearest an unrelated paragraph is the same fabrication as inventing an answer,
and it is the easier mistake to ship, because it looks like helpfulness.
...
backend/scripts/         # corpus generator, db inspector, screenshot capture
frontend/                # React app
docs/                    # API + function reference
docs/images/             # README screenshots, regenerated by a script
...
**The queue a lead triages, and the number to watch.**

| Review queue | Measurement |
|---|---|
| [<img src="docs/images/review-queue.png" width="410">](docs/images/review-queue.png) | [<img src="docs/images/measurement.png" width="410">](docs/images/measurement.png) |
| Every routed question, joined to the query that caused it, with the routee and a resolve control. | The first-30-days metric beside its live value, the trailing window that is the actual early-warning signal, and ingestion health — which correctly surfaces the two deliberately-degraded documents at 0.22 and 0.26. |

The headline on that Measurement shot reads **40% against a 70% target**, and
that number is real but not meaningful: the only traffic in the log is the
three questions used to take these screenshots, two of which are deliberately
unanswerable. Six routed out of ten. It is left as captured rather than staged
against a flattering query mix, which would be a strange thing to fake in a
README about not fabricating answers.

All seven images are regenerated from the running app by
[`backend/scripts/capture_screenshots.py`](backend/scripts/capture_screenshots.py),
for the same reason the Office corpus is generated rather than hand-committed:
a screenshot nobody can reproduce is a claim, not evidence. Re-run it after a
UI change and the README stops lying instead of quietly aging.
...
## Answer quality: the golden gate
